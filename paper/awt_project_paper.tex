...
\end{figure}
After the segmentation request was send, a recursive receive via \textit{setTimeout()} is called every 100 milliseconds.
...
The implemented approach polls for merely one message at a time and only considers it, if the identified \textit{jobID} is targeting the user.
In case one segmentation is done, the message contains an MPD URL, which is saved temporarily and the next chunks will be segmented, but only if their corresponding source URLs have not been segmented yet.
Otherwise, a progress per chunk segmentation is shown and the next message is polled.
Either way, that message is then removed from the queue by its proper recipient so that the next results in the FIFO queue can be received.

In this prototype version of the UI, a ``purge\&restart''-button was implemented as a safety measure.
Should any client abort the message receiving process so that the result queue won't make any progresses, this button will remove all messages on the queue and restart the FOKUS backend server.
\\
...
Except the message to be transmitted, sending and receiving a stitching request is analog to that of a segmentation request.
\begin{figure}[H]
\begin{lstlisting}
var configStitching = {
  "LOG_LEVEL": "info",
...
Since there was no documentation of the stitching backend and because its developers were not always contactable, this issue could not be resolved at the moment.
...
- if client dies or abort receiving while not read all msg -$>$ other clients can not receive their msg from queue \\
- if no client dies while sending / receiving then should work perfectly for multiple parallel requests \\
...
Shortcomings: quantization 100ms -$>$ video ende abgeschnitten, 10ms -$>$ schwächere cpu schaffen nicht; same for SQS polling \\
